...
class EditRole extends StatefulWidget {
  final RolesReadResponse rolesReadResponse;

  const EditRole({Key key, this.rolesReadResponse}) : super(key: key);
...
class _EditRoleState extends State<EditRole> {
  ...
  void initState() {
    ...
    setState(() {
      _nameController.text = widget.rolesReadResponse.role;
      accessIdMap = widget.rolesReadResponse.accessIdMap;
    });
  ...
  _getRolePermissionList() {
    HTTPManager().getRolesPermissions(accessIdMap).then((value) {
      setState(() {
        _isLoading = false;
                        ...
                        rows: List.generate(
                            ...
                            (index) => DataRow(cells: <DataCell>[
                                  ...
                                  DataCell(rolesPermissions[index]
                                      ...
                                      onChanged: (val) {
                                        setState(() {
                                          rolesPermissions[index].read = val;
                                        });
                                        _updateAccessId(
    ...
    if (_roleFormKey.currentState.validate()) {
      ...
      HTTPManager()
          .updateRole(RoleUpdateRequest(
              id: widget.rolesReadResponse.id,
              companyId: widget.rolesReadResponse.companyId,
